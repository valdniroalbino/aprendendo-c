...
void processarVoto(Resultado *r,Pilha *auditoria,Voto voto,Candidato *candidatos){
    empilhar(auditoria,voto);

    if(voto.idCandidato == 0){
        r->brancos++;
    }
    else if(voto.idCandidato < 0){
        r->nulos++;
    }
    else{
        Candidato *c=procurarCandidato(candidatos,voto.idCandidato);
        if(c){
            r->validos++;
            c->votos++;
        }else{
